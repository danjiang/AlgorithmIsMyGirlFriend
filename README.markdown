...
- [最长递增子序列](/SkillAndThought/longest_increase_subsequence.cpp)：有一个数字序列包含 n 个不同的数字，如何求出这个序列中的最长递增子序列长度？比如 2, 9, 3, 6, 5, 1, 7 这样一组数字序列，它的最长递增子序列就是 2, 3, 5, 7，所以最长递增子序列的长度是 4。这个问题可以转换为最长公共子序列问题，如例子中的数组 A { 5, 6, 7, 1, 2, 8 }，则我们排序该数组得到数组 A‘ { 1, 2, 5, 6, 7, 8 }，然后找出数组 A 和 A’ 的最长公共子序列即可。还可以倒着分析，得出 logn 的解法。
