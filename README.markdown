...
- [n 个骰子的点数：把 n 个骰子仍在地上，所有骰子朝上一面的点数之和为 s，输入 n，打印出 s 的所有可能的值出现的概率。](RecursionAndLoop/fibonacci.cpp)
...
| [快速排序](SortAndFind/quick_sort.cpp) | 是 | 是 | 线性对数阶 O(nlogn) | 平方阶 O(n2) | 线性对数阶 O(nlogn) |

- 归并排序的空间复杂度是 O(n)。
- 通过合理地选择 pivot 来避免快速排序算法时间复杂度退化到 O(n2)。
